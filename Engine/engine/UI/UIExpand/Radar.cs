...
public class Radar : MaskableGraphic, ISerializationCallbackReceiver, ICanvasRaycastFilter
{
    [SerializeField]
    ...
    [SerializeField]
    public List<float> m_Weights = null;
    ...
    public void SetRadarWeight ( int index, float value )
    {
        if (index >= 0 && index < Weights.Count && value <= 1f && value >= 0f)
        {
            Weights[index] = value;
            SetVerticesDirty();
        }
    ...
    private void GetTriangle ( VertexHelper vh, int index, float deltaAngle )
    {
        ...
        float radius1 = ( index == EdgeCount - 1 ? Weights[0] : Weights[index + 1] ) * edgeLength;
