...
namespace UEngine.FreeType
{
    ...
    private const string s_library = "__Internal";
        ...
        [DllImport(s_library)]
        public static extern IntPtr UFT_GetGlyph ( IntPtr pContext, int code, ref FreeTypeGlyph glyph, int fontsize, int outlinesize, bool bold, bool italic );
        ...
        public static IntPtr GetGlyph(IntPtr fontContext, int charCode, out FreeTypeGlyph glyph, int fontsize, int outlinesize, bool bold, bool italic)
        {
            IntPtr result = IntPtr.Zero;
            ...
            {
                result = UFT_GetGlyph( fontContext, charCode, ref glyph, fontsize - UIManager.OffsetSize, outlinesize, bold, italic );
            }
            else
            {
                ULogger.Warn( charCode + " :不存在字体中 outlinesize: " + outlinesize );
            }
            return result;
        }
    }
